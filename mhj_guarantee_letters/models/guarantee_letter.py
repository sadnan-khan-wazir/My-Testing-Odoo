    ...
    def confirm_button(self):
        for rec in self:
            ...
            if rec.is_expenses :
                ...
                self.env['account.move.line'].with_context(check_move_validity=False).create(
                    {
                        'move_id': move.id,'account_id': rec.journal_id.default_account_id.id,'name': 'Gurantee Letter','analytic_account_id': rec.analytic_id.id,'credit': rec.expenses_amount,})
                move.post()
