...
class SaleOrderLine(models.Model):
    ...
    pack_line_price_unit = fields.Float(string='Pack Line price')
    ...
    def expand_pack_line(self, write=False):
        ...
        if self.product_id.pack_ok and self.pack_type == "detailed":
            for subline in self.product_id.get_pack_lines().filtered(
                    lambda pl: pl.product_id.service_type_id in self.order_id.service_type_ids):
    ...
    def action_open_parent_pack_product_view(self):
        domain = [("id", "in", self.mapped("pack_parent_line_id").mapped("product_id").ids)]
