...
class ProductPack(models.Model):
    ...
    def get_sale_order_line_vals(self, line, order):
        self.ensure_one()
        self = self.filtered(lambda l: l.product_id.service_type_id in order.service_type_ids)
        ...
        if line.product_id.pack_component_price == "detailed":
            ...
            # Keep line price in other field 
            vals["pack_line_price_unit"] = vals["price_unit"]
            vals["price_unit"] = 0.0
        vals.update(
            {
